...
from core.utilities import all_unique
...
class node(object):
    ...
    __slots__ = ["space_time", "future", "past", "left", "right", "index", "phi", "t"]
    ...
    def same_t(self):
        eq_right = self.t == self.space_time.get_node(self.right).t
        eq_left = self.t == self.space_time.get_node(self.left).t
        return eq_right and eq_left
    ...
    def validate(self):
        ...
        if not hassame_t:
            print("{}: has a different time from its left and or right".format(self))
